...
import { mapHttpErrorToAuthError } from "../../domain/auth-error";
...
function assertOnlineOrThrow(): void {
  if (!navigator.onLine) {
    const offlineError = mapHttpErrorToAuthError(
...
async function persistSessionOrThrow(response: AuthResponse): Promise<void> {
  try {
    await saveAuthResponse(response);
  } catch (error) {
    const authError = mapHttpErrorToAuthError(
      error instanceof Error ? error : new Error("saveAuthResponse failed"),
      navigator.onLine
    );
    throw new HttpError(
      authError.message,
...
const mockAdapter: AuthAdapter = {
  async login(credentials) {
    ...
    await persistSessionOrThrow(response);
    return response;
  },
