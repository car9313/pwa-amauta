...
## Fase 5: Offline Mode con Token Expirado (07/06/2026)

### Cambio
Cuando el access token expira mientras la app está offline, **ya no se borra la sesión**. En su lugar:
- El usuario sigue autenticado (`isAuthenticated = true`) para que las rutas protegidas funcionen
- Se marca `isOfflineMode = true` y `lastAuthError = "TOKEN_EXPIRED"`
- Al reconectar, se intenta refrescar automáticamente usando el refresh token almacenado

### Problema que resuelve
Antes, `hydrateFromStorage()` llamaba a `clearAuth()` cuando el token estaba expirado, borrando user + tokens de IndexedDB. Esto forzaba un redirect a `/login` aunque el usuario tuviera datos locales para ver en modo offline.

### Archivos modificados

#### 1. `src/features/auth/presentation/store/auth-store.ts` — `hydrateFromStorage()`
```typescript
// Antes: token expirado → clearAuth() → redirect a /login
if (!isValid) {
  await clearAuth();
...
// Después: token expirado → mantener sesión, entrar en offline mode
if (!isValid) {
  const stored = await loadAuthFromStorage();
  if (stored?.user) {
    set({ isAuthenticated: true, user: stored.user, isOfflineMode: true, lastAuthError: "TOKEN_EXPIRED" });
  } else {
    await clearAuth();  // Solo si no hay datos locales
  }
  return;
}
```

#### 2. `src/features/auth/hooks/useOfflineMode.ts`
- `handleOnline()` ahora intenta `refreshAccessToken()` cuando detecta `isOfflineMode && lastAuthError === "TOKEN_EXPIRED"`
- Si el refresh es exitoso → limpia error y offline mode
- Si falla → `clearSession()` (sesión inválida)
- Antes simplemente limpiaba el error sin intentar refresh

#### 3. `src/lib/sync/queue-manager.ts` — `processQueue()`
- Antes de procesar mutaciones encoladas, verifica si hay token expirado
- Si es así, intenta refresh antes de hacer cualquier request HTTP
- Si el refresh falla, la cola no se procesa (lanza error)

#### 4. `src/features/auth/domain/auth-error.ts`
- Mensaje de `TOKEN_EXPIRED` actualizado a: "Tu sesión expiró. Conéctate a internet para renovarla automáticamente."

### Diagrama de flujo

```
App inicia con token expirado
    │
    ▼
hydrateFromStorage()
    │
    ├─ ¿User en Dexie? → Sí → isAuthenticated=true, isOfflineMode=true
    │                           └─ Usuario navega la app en modo offline
    │
    ├─ ¿User en Dexie? → No → clearAuth() → redirect /login
    │
    ▼
Usuario vuelve a internet
    │
    ▼
useOfflineMode.handleOnline()
    │
    ├─ refreshAccessToken() exitoso → limpia error, offline mode=false
    │                                  └─ App funciona normalmente
    │
    └─ refreshAccessToken() falla → clearSession() → redirect /login
...
processQueue()
    │
    ├─ ¿TOKEN_EXPIRED? → intenta refresh
    │   ├─ éxito → procesa cola normalmente
    │   └─ fallo → lanza error, no procesa
    │
    └─ no hay error → procesa cola normalmente
```

### Consideraciones con backend no implementado (mock mode)

⚠️ **Limitación importante**: `refreshAccessToken()` en mock mode **siempre tiene éxito** (refresh.ts:18-23):

```typescript
if (USE_MOCKS || !API_BASE_URL) {
  // Siempre retorna un nuevo mock token, NUNCA falla
  return { access: "mock_access_" + Date.now(), expiresIn: 900 };
}
```

Esto implica:

| Escenario | Mock mode | Real backend |
|-----------|-----------|--------------|
| handleOnline → refresh | **Siempre exitoso** → limpia error | Puede fallar → clearSession |
| processQueue → refresh | **Siempre exitoso** → procesa cola | Puede fallar → lanza error |
| hydrateFromStorage | Funciona normal | Funciona normal |

**Path no testeable en mock mode**: El caso donde el refresh token expiró o fue revocado, y `handleOnline`/`processQueue` llaman a `clearSession()`. Para probar este path se necesita un backend real o modificar el mock de `refreshAccessToken()` para que falle condicionalmente.

### Cómo probar el flujo offline

#### En mock mode (solo happy path)
```bash
# 1. Iniciar sesión con cualquier mock user
#    → Login guarda tokens + user en Dexie

# 2. Forzar expiración del token
#    → Abrir DevTools > Application > IndexedDB > amauta-db
#    → Modificar expiresAt a Date.now() - 1000

# 3. Recargar la app (F5)
#    → hydrateFromStorage detecta token expirado
#    → isOfflineMode=true, lastAuthError="TOKEN_EXPIRED"
#    → El usuario ve la app (no redirige a login)

# 4. El refresh se intenta automáticamente
#    → En mock mode siempre funciona (limpiando el error)
```

#### Con real backend (happy + error path)
```bash
# 1. VITE_USE_MOCK=false con backend corriendo
# 2. Login → obtener tokens reales
# 3. Esperar a que el access token expire (~15 min por defecto)
# 4. Poner navegador offline
# 5. Recargar → hydratación mantiene sesión con isOfflineMode=true
# 6. Poner navegador online → handleOnline intenta refresh
#    → Si refresh token es válido: limpia error, app normal
#    → Si refresh token expiró: clearSession(), redirect a login
```