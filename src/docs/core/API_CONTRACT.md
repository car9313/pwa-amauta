 ...
 Respuesta (200 OK):
{
  ...
  "exercise": {
    ...
    "subInstruction": "¡Hazlo como en la pizarra!"
  },
...
Cuando el dispositivo está sin conexión, la mutación se encola en IndexedDB (Dexie) y el frontend recibe el símbolo `QUEUED_OFFLINE` en lugar del `ExerciseResult`. La sincronización ocurre automáticamente al recuperar la conexión vía `background-sync.ts`.

---

## Flujo de Ejercicio Completo
...
Frontend                          Backend
   ...
   │                                 │
   │  navigate(/lessons/feedback)    │
   │  (FeedbackPage consume result)  │
   │                                 │
   │  "Siguiente" → /lessons         │
   │  (ciclo se repite)              │
```

El `ExerciseResult` se pasa al `FeedbackPage` via `location.state` de React Router.

### Posibles estados del `ExerciseResult`

| score | passed | UI en FeedbackPage |
|-------|--------|-------------------|
| >= 80 | true | Celebración + "¡Excelente!" + botón Siguiente |
| 70-79 | true | Ánimo + "¡Buen trabajo!" + botón Siguiente |
| < 70 | false | Feedback correctivo + mistakes + botón Intentar de nuevo |
