...
| `src/lib/sync/queue-manager.ts` | Lógica de cola, ejecución HTTP, procesamiento por prioridad | `queueMutation`, `processQueue`, `triggerSync`, `getQueueState`, `isOnline`, `clearQueue` |
...
| `src/lib/sync/conflict.ts` | Estrategias de resolución de conflictos | `lastWriteWins`, `resolveConflict`, `serverWins`, `clientWins` |
...
6. Por cada mutation: ejecuta HTTP, si éxito elimina, si falla incrementa retryCount
7. Después de 3 fallos → `status: "failed"`
